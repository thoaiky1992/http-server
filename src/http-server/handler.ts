...
import { authMiddleware } from '~src/middlewares/auth.midlleware';
...
async function findControllersDirAndReadFiles(dir: string) {
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === 'controllers') {
          console.log(`Found "controllers" directory: ${fullPath}`);
          await readFilesInDirectory(fullPath);
        } else {
          await findControllersDirAndReadFiles(fullPath);
        }
      }
    }
  } catch (err) {
    console.error(`Error reading directory ${dir}:`, (err as any).message);
  }
}

async function readFilesInDirectory(dir: string) {
  try {
    ...
    console.error(`Error reading files in ${dir}:`, (err as any).message);
  }
}
...
export async function initializeMiddlewareAndRoute(app: Express, prefixApi: string) {
  ...
  controllerPaths.forEach((controllerPath: any) => {
    ...
    routes.forEach((route: RouteType) => {
      ...
      // console.log({ isAuthForBaseRoute });
      if (classRequiresAuth || methodRequiresAuth || isAuthForBaseRoute) {
        route.middlewares.unshift(authMiddleware);
      }
      app[method](
