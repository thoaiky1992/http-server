...
import ErrorMiddleware, { TErrorMiddleware } from '~src/middlewares/error.middleware';
...
export class HttpServer extends IHttpServer {
  ...
  public async start(): Promise<void> {
    ...
    initializeMiddlewareAndRoute(this._app, this.prefixApi);
  ...
  public applyErrorMiddleware(errorMiddleware: TErrorMiddleware) {
  ...
  public async listen(): Promise<void> {
    await new Promise<void>((resolve) =>
      this._http.listen(this.port, () => {
        ...
        Logger.info(
          'Http',
          `🚀 Server ready at http://localhost:${this.address.port}${normalizePath(this.prefixApi, '')}`
        );
        resolve();
      })
    );
